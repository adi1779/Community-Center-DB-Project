...
SELECT CONCAT(FirstName, ' ', LastName) AS "Full Name",
       JoiningDate,
       TIMESTAMPDIFF(YEAR, JoiningDate, CURDATE()) AS "Seniority (Years)"
FROM members;

-- 2
SELECT * FROM instructors
WHERE (LastName LIKE 'G%')
AND (Speciality IS NULL OR InstructorID BETWEEN 1 AND 50);

-- 3
SELECT MemberID,
       SUM(FinalPrice) AS "Total Revenue",
       AVG(FinalPrice) AS "Average Price",
       MAX(FinalPrice) AS "Highest Transaction"
FROM payments
GROUP BY MemberID
HAVING SUM(FinalPrice) > 100;
...
WHERE l.InstructorID IN (
    SELECT InstructorID 
    FROM Instructors
    WHERE Speciality LIKE 'Yoga%' OR Speciality IS NULL
)
AND m.JoiningDate BETWEEN '2025-01-01' AND CURDATE()
AND m.PhoneNumber IS NOT NULL
ORDER BY m.LastName ASC;

-- 5
DELETE FROM Registrations 
WHERE RegistrationID = ANY (
    SELECT id FROM (
        SELECT RegistrationID AS id 
        FROM Registrations 
...
BEGIN
    ...
    IF NEW.Email != OLD.Email THEN
        IF NEW.Email NOT LIKE '%@%' THEN
            ...
            SET MESSAGE_TEXT = 'Error: Invalid Email format! Must contain @';
        END IF;
    END IF;
END //
DELIMITER ;
...
SELECT FirstName, LastName, FinalPrice AS "Original", AddTax(FinalPrice) AS "Price after tax"
FROM members
JOIN payments ON members.MemberID = payments.MemberID;

-- 8
DELIMITER //
CREATE PROCEDURE UpdateLargePaymentsWithoutOutput(IN m_id INT, IN new_method VARCHAR(20))
BEGIN
    UPDATE payments
    SET PaymentMethod = new_method
    WHERE MemberID = m_id AND FinalPrice > 100;

    SELECT MemberID, FinalPrice, PaymentMethod, PaymentDate
    FROM payments
    WHERE MemberID = m_id;
END //
DELIMITER ;
...
-- 9
DELIMITER //
CREATE PROCEDURE UpdateNameAndShow(IN m_id INT, IN new_name VARCHAR(50))
BEGIN
...
-- 10
DROP VIEW IF EXISTS VwPaymentsPerMember;
CREATE VIEW VwPaymentsPerMember AS
SELECT members.MemberID, members.FirstName, payments.FinalPrice
FROM members JOIN payments ON members.MemberID = payments.MemberID;


SELECT MemberID, FirstName, FinalPrice AS highest_payment_amount
FROM VwPaymentsPerMember
WHERE FinalPrice = (SELECT max(FinalPrice) FROM VwPaymentsPerMember);